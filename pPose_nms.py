...
def pose_nms(bboxes, bbox_scores, pose_preds, pose_scores):
    ...
    widths = xmax - xmin
    heights = ymax - ymin
    ref_dists = alpha * np.maximum(widths.cpu(), heights.cpu())
    ...
    while human_scores.shape[0] != 0:
        ...
        bbox_scores = np.delete(bbox_scores.cpu(), delete_ids, axis=0)
    ...
    for j in range(len(pick)):
        ...
        final_result.append({
            ...
            'proposal_score': torch.mean(merge_score).cpu() + bbox_scores_pick[j].cpu() + 1.25 * max(merge_score).cpu()
        })
